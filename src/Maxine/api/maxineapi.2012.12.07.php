<?PHP
	...
	// Dashboard functions {
		...
		function findPageDimensionFactor(array $dimensions) {
			...
			//: Browser top bar
			if ($wide) {
				$factor = (float)$factor-(($dimensions["height"]["screen"]-$dimensions["height"]["window"])/1080);
			}
			//: End
		...
		*/
		...
		function get_gravatar($email, $cache_locally = false, $s = 80, $d = 'mm', $r = 'g', $img = false, $atts = array()) {
		    ...
		    if (isset($cache_locally) && (is_bool($cache_locally) === TRUE) && ($cache_locally === TRUE)) {
		        ...
		        curl_setopt($curl, CURLOPT_PROXYUSERPWD, "feighen:fgno456");
		        $data = curl_exec($curl);
