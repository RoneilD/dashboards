<?PHP
	function displayMainDash() {
		...
		print("<table style='width:100%; height:100%; cursor: none;' background='".BASE."/images/background.png' cellspacing=0 cellpadding=0 border=0>");
	...
	function displayNoDash() {
		print("<body style='padding:0px; background-color:BLACK;'>");
	...
	function displayMyDash() {
		...
		print("<body style='margin:0px; padding:0px; background-image:url(\"".BASE."/images/background.png\");'>");
		
		
		print("<h1 style='background-image:url(\"".DISPLAYCASE."/header_bar.png\"); margin-top:0px; width:100%; height:92px;'>");
		...
		// Control bar {
			...
			print("<img src='".DISPLAYCASE."/settings.png' onclick='goTo(\"index.php?mode=maxine/index&action=mydashdetails\");' style='margin-left:15px; margin-top:10px;'>");
			print("<img src='".DISPLAYCASE."/home.png' onclick='goTo(\"index.php?action=home\");' style='margin-left:15px; margin-top:10px;'>");
		...
		// forward/rewind/pause bar {
			...
			print("<img src='".DISPLAYCASE."/dashcontrols/back.png' onClick='rewindSequence();' style='margin-left:25px;'>");
			print("<img id='pausebutton' src='".DISPLAYCASE."/dashcontrols/pause.png' onClick='pausePressed();' style='margin-left:15px; width:31px; height:51px;'>");
			print("<img id='playbutton' src='".DISPLAYCASE."/dashcontrols/play.png' onClick='playPressed();' style='margin-left:15px; display:none;'>");
			print("<img src='".DISPLAYCASE."/dashcontrols/forward.png' onClick='fasttrackSequence();' style='margin-left:15px;'>");
		...
		*/
		...
		// Javascript {
			print("<script>
				".$scriptstr."
				var cycleCounter = 0, cycleduration = ".($duration * 1000).", max = ".$patterncount.", screenwidth = screen.width, interval1 = setInterval('ajaxTicker()', cycleduration);
				ajaxTicker();
				
				function ajaxTicker() {
					var ajaxRequest;  // The variable that makes Ajax possible!
					// Rip the records variables into a string for Posting {
						var params	= 'fleetcount='+patternscript[cycleCounter];
						params		+= '&maxwidth='+screenwidth;
	...
	// Personal Dash functions {
		function myDashDetails() {
			// Prep {
				...
				$selectstr2	.="][fleetid] style=\'width: 90%;\'><option value=100>Fleet Comparison by Day</option><option value=101>Fleet Comparison by Month</option>";
			...
			print("<link href='".BASE."/basefunctions/scripts/manline.css' media='all' rel='stylesheet' type='text/css' />".PHP_EOL);
			print("<title>Maxine</title>".PHP_EOL);
			print("<body id='body' style='margin:0px; padding:0px;'>");
			
			print("<form name='dashdetailsform' action='index.php?mode=maxine/index&action=updatemydashdetails' method='post'>");
			
			print("<table width=100% height=100% cellspacing=0 cellpadding=0 border=0 background='".BASE."/images/background.png'>");
	...
	function checkFleetScoreUpdates() {
		...
		*/
		...
		// Selector {
			...
			print("<select name='conf[fleetid]' value='".$fleetid."' style='width:180px;'>");
			...
			print("Start Date");
			print("</td><td align='center'>");
			print("<input name='conf[startdate]' value='".date("d/m/Y", $startdate)."' readonly style='width: 160px; text-align: center;'>");
			print("<img src='".BASE."/images/calendar.png' onClick='displayDatePicker(\"conf[startdate]\", this, \"dmy\", \"\");'>");
			print("</td></tr>");
			...
			print("Stop Date");
			print("</td><td align='center'>");
			print("<input name='conf[stopdate]' value='".date("d/m/Y", $stopdate)."' readonly style='width: 160px; text-align: center;'>");
			print("<img src='".BASE."/images/calendar.png' onClick='displayDatePicker(\"conf[stopdate]\", this, \"dmy\", \"\");'>");
			print("</td></tr>");
		...
		if($updatelist) {
			...
			print("<table class='standard' style='width:1100px; margin-bottom:20px;'>");
			
			// Headers {
				print("<tr class='heading'><td width=8%>");
				print("ID");
