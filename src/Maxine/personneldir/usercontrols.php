	...
	// User Functions {
		function listUsers() {
			...
			// Preparation {
				...
				$users->setQueryColumns(array(
					"users"=>array("personid", "user_profiles_id", "username", "email", "extension", "cell", "isplace", "isgeneric", "deleted"),
					"user_profiles"=>array(
						"firstname","lastname",
						"jobtitle", "staffno", "id", "birthday", "department_id", "location", "interests", "family", "aspirations",
						"goals", "quote", "createDate"
						)
					));
		...
		function editUserForm() {
			...
			// Data Prep {
				...
				$profiles->setQueryColumns(array(
					"user_profiles"=>array(
		...
		function commitUser() {
			...
			if($conf["personid"]) {
				if (array_key_exists("password", $conf) && $conf["password"])
                                {
                                        $conf["password"] = md5("bWLMq4c".$conf["password"]);
                                }
