<?PHP
	...
	*/
	...
	class UnitFaults extends Table {
		// Variables {
			protected $_cols = array();
			protected $_dependantTables = array(
			);
			protected $_children = array(
				"uf_notes"=>array("table"=>"uf_notes", "cols"=>"*", "customkey"=>"faultid")
			);
			protected $_title	= "unitfault";
			protected $_name	= "uf_faults";
			protected $_primary;
		// }
		...
		// Get one record from Unit Faults {
			/** UnitFaults::getRow($options)
			* @param $options array
			...
			public function getRow(array $options) {
				...
				if ($options["children"]) {
					foreach ($this->_children as $childkey=>$childval) {
						...
						$child["where"] = "faultid=".$data["unitfaultid"];
		...
		/** UnitFaults::setVariables($config)
		* @param $config array
