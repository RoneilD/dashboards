		...
		function myDashDetails() {
			// Prep {
				...
				if ((isset($_SESSION["isit"]) && $_SESSION["isit"]>0) || (isset($user["ismanager"]) && $user["ismanager"]>0)) {
					$selectstr2	.="<option value=100>Fleet Comparison by Day - Revenue</option>";
					$selectstr2	.="<option value=150>Fleet Comparison by Day - Contribution</option>";
					$selectstr2	.="<option value=101>Fleet Comparison by Month - Revenue</option>";
				...
				foreach ($fleetlist as $fleetkey=>$fleetval) {
					if ((isset($_SESSION["isit"]) && $_SESSION["isit"]>0) || (isset($user["ismanager"]) && $user["ismanager"]>0)) {
						$selectstr2	.= "<option value=\"".$fleetkey."\">".$fleetval["name"]." - Revenue</option>";
				...
				// Drag and drop table {
					...
					foreach ($pattern as $patternkey=>$patternval) {
						...
						// Fleet Select {
							...
							if ((isset($_SESSION["isit"]) && $_SESSION["isit"]>0) || (isset($user["ismanager"]) && $user["ismanager"]>0)) {
								print("<option value=100 ".(100==$patternval?"selected":"").">Fleet Comparison by Day - Revenue</option>");
								print("<option value=150 ".(150==$patternval?"selected":"").">Fleet Comparison by Day - Contribution</option>");
								print("<option value=101 ".(101==$patternval?"selected":"").">Fleet Comparison by Month - Revenue</option>");
							...
							foreach ($fleetlist as $fleetkey=>$fleetval) {
								if ((isset($_SESSION["isit"]) && $_SESSION["isit"]>0) || (isset($user["ismanager"]) && $user["ismanager"]>0)) {
									print("<option value='".$fleetkey."' ".($fleetkey==$patternval?"selected":"").">".$fleetval["name"]." - Revenue</option>");
